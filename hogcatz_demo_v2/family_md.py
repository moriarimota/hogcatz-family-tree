	...
	- [席琳（波伏娃）](https://mp.weixin.qq.com/s/c3mnL9DFpV44N6C3uErsfQ)，女孩
		...
		- [黑塞](https://mp.weixin.qq.com/s/2v50kJZeko81t2goZietdw)，男孩
			...
			- [毛毛（伍尔芙）](https://mp.weixin.qq.com/s/QK8EP5hIqtoa-wdn8XFLzA)，女孩
				...
				- [雪饼（柯莱特）](https://mp.weixin.qq.com/s/as-YiXH-zLicMZdtucfU-w)，女孩
			...
			- [铒居（蒙布朗）](https://mp.weixin.qq.com/s/kzz4c-XV5YvVB61eZAz_Mg)，男孩
				...
				- [卡隆（马卡龙）](https://mp.weixin.qq.com/s/V4EnIl_JzRvXpIDtG3Y8eA)，男孩
					...
					- [玛德琳](https://mp.weixin.qq.com/s/PeVCHUwlO4HdI9z1bxzZ4w)，男孩
						...
						- [司康](https://mp.weixin.qq.com/s/iSj0GlVNT09k7tQrUkH1Xg)，男孩
		...
		- [乐乐（牛肝菌）](https://mp.weixin.qq.com/s/eJYnUO3q5F5fWsPP-mKa8A)，男孩
			...
			- [福福（姬松茸）](https://mp.weixin.qq.com/s/Lai9gUPqEqy8gZTBxl4MgQ)，女孩
				...
				- [花花（虫草花）](https://mp.weixin.qq.com/s/3o7cNDZGK9xPFEaShLsHIQ)，女孩
					...
					- [Gimi（黑松露）](https://mp.weixin.qq.com/s/2bw1y7tcDkN-yIKBcIrt-A)，男孩
...
def add_cat_node(name, gender, link=None):
    ...
    if name not in nodes:
        nodes[name] = {
            ...
            "link": link or "",
            "type": "cat",
            "gender": "male" if "男" in gender else "female"
        }

def add_litter_node(litter_id, litter_name):
    if litter_id not in nodes:
        nodes[litter_id] = {
            "id": litter_id,
            "name": litter_name,
            "type": "litter"
...
for line in lines:
    ...
    if line.startswith("## "):
        current_father = clean_name(line[3:].split("（")[0])
        add_cat_node(current_father, "男孩")

    elif line.startswith("### "):
        current_mother = clean_name(line[4:].split("（")[0])
        add_cat_node(current_mother, "女孩")

    elif re.match(r"^- \d{4}/\d{1,2}/\d{1,2}", line) or re.match(r"^- \[?\d{4}/\d{1,2}/\d{1,2}", line):
        litter_id = f"窝_{litter_count}"
        litter_count += 1
        current_litter = litter_id
        add_litter_node(litter_id, re.sub("^- ", "", line))
        # 添加配对关系
        if current_father and current_mother:
            links.append({
                "source": current_father,
                "target": litter_id,
                "type": "mate",
        ...
        if match:
            if len(match.groups()) == 3:
                ...
                link = None
            name = clean_name(name)
            add_cat_node(name, gender, link)
            if current_litter:
                links.append({
                    "source": current_litter,
                    "target": name,
                    "type": "child",
...
print(json.dumps(graphData, ensure_ascii=False, indent=4))
